...
function(bazel cmake_target cmake_visibility bazel_target)
  ...
  execute_process(
    COMMAND ${BAZEL_PYTHON_PATH} ${_THIS_MODULE_BASE_DIR}/bazel.py ${bazel_target}
      ${_BAZEL_DEBUG}
      ${_BAZEL_DEBUG_MESSAGE_LIMIT}
      ${_BAZEL_ARGS}
      ${_BAZEL_SHELL}
      ${_BAZEL_NO_BUILD}
  ...
  string(REGEX MATCH "INCLUDE_DIRS=.*" INCLUDE_LINE "${DEPENDENCIES}")
  string(REPLACE "INCLUDE_DIRS=" "" INCLUDE_LINE "${INCLUDE_LINE}")
  separate_arguments(INCLUDE_DIRS NATIVE_COMMAND "${INCLUDE_LINE}")
  target_include_directories(${cmake_target} ${cmake_visibility} ${INCLUDE_DIRS})

  string(REGEX MATCH "LINK_DIRS=.*" LINK_LINE "${DEPENDENCIES}")
  string(REPLACE "LINK_DIRS=" "" LINK_LINE "${LINK_LINE}")
  separate_arguments(LINK_DIRS NATIVE_COMMAND "${LINK_LINE}")
